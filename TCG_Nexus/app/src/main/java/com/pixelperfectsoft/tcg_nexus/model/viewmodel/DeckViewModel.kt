...
class DeckViewModel : ViewModel() {
    private var state: MutableState<DataState> = mutableStateOf(DataState.Empty)
    val decks = mutableStateOf(Collect())
    ...
    init {
        /*
            Este metodo se va a ejecutar cuando se instancie la clase DeckViewModel
         */
        get_decks()
    }
    ...
    fun get_decks() {
        viewModelScope.launch {
            decks.value = retrieveDecks()
            show_cards_from_collection()
        }

    }

    suspend fun retrieveDecks(): Collect {
        /*
        ...
        try {
            FirebaseFirestore.getInstance()
                ...
                .get().await().map {
                    ...
                    if (userId != null) {
                        Log.d("collection-retrieve", "Collection found -> $decks")
    ...
    fun show_cards_from_collection() {
        ...
        * 4. Obtenemos cartas de la Realtime Database de Firestore y las añadimos al array temporal
         */
        val collection = decks.value.cards
